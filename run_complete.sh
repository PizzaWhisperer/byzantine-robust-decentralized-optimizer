...
function optimization_delta {
  ...
  #  do
    ...
    do
        ...
        --graph c5,1,1 --noniid 1 --agg "scp1" --identifier "exp"
        #&
        #pids[$!]=$!
    done
...
function optimization_delta_plot {
    COMMON="--lr 1e-3 --use-cuda --epochs 50 --batch-size 32 --max-batch-size-per-epoch 30"
    python optimization_delta_complete.py ${COMMON} -n 12 -f 1 --attack "BF" --momentum 0.9 \
    --graph c5,1,0 --noniid 1 --agg "scp1" --identifier "exp" \
    --analyze
}
...
function honest_majority {
    ...
    do
      ...
      do
        ...
        --graph c5,1,0 --noniid 0 --agg ${agg} --identifier "exp"
        #&pids[$!]=$!
      done
    done
    ...
    for agg in "scp0.1" "rfa8" "mozi0.4,0.5" "tm2"
      ...
      do
        ...
        --graph c5,1,1 --noniid 0 --agg ${agg} --identifier "exp" #&
        #pids[$!]=$!
      done
    done
...
function honest_majority_plot {
    ...
    --graph c5,1,1 --noniid 0 --agg "scp1" --identifier "exp" --analyze
}
