...
function optimization_delta {
    ...
    do
        ...
        for attack in "scc_stateoverride" "stateoverride"
        do
          for f in 1
          do
            python optimization_delta.py ${COMMON} -n 12 -f $f --attack $attack --momentum 0.9 \
        ...
        do
          python optimization_delta.py ${COMMON} -n 12 -f $f --attack $attack --momentum 0.9 \
          --graph tcb5,0,${delta} --noniid 1 --agg "scp1" --identifier "exp"
        done
    done
}
